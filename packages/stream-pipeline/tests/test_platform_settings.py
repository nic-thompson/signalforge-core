"""
Tests for stream_pipeline.config.platform_settings.
...
from stream_pipeline.config.platform_settings import (
...
class PlatformSettingsDatasetBucketValidationTest(unittest.TestCase):
    ...
    def test_accepts_valid_bucket_name(self) -> None:
        settings = PlatformSettings(dataset_bucket="stream-pipeline-prod")
        self.assertEqual(settings.dataset_bucket, "stream-pipeline-prod")
    ...
    def test_rejects_bucket_name_starting_with_hyphen(self) -> None:
        with self.assertRaises(ValueError):
            PlatformSettings(dataset_bucket="-stream-pipeline")
...
class PlatformSettingsFromEnvTest(unittest.TestCase):
    ...
    def test_from_env_loads_dataset_buckets(self) -> None:
        settings = PlatformSettings.from_env(
            env={
                "SF_DATASET_BUCKET": "stream-pipeline-prod",
                "SF_REPLAY_DATASET_BUCKET": "stream-pipeline-replay",
            }
        )
        self.assertEqual(settings.dataset_bucket, "stream-pipeline-prod")
        self.assertEqual(settings.replay_dataset_bucket, "stream-pipeline-replay")
...
class PlatformSettingsReplayTest(unittest.TestCase):
    ...
    def test_for_replay_swaps_dataset_bucket(self) -> None:
        live = PlatformSettings(
            dataset_bucket="stream-pipeline-prod",
            replay_dataset_bucket="stream-pipeline-replay",
        )
        replay = live.for_replay()
        self.assertEqual(replay.dataset_bucket, "stream-pipeline-replay")

    def test_for_replay_swaps_to_none_when_no_replay_bucket(self) -> None:
        live = PlatformSettings(dataset_bucket="stream-pipeline-prod")
        replay = live.for_replay()
