...
- **Severity-to-target dispatch.** The router carries severity; the EventBridge bus rules (infra) route on it. stream-pipeline does not decide which target a CRITICAL reaches.
...
- `stream_pipeline/alerts/` — the layer: `alert_router.py`, `acknowledgement_registry.py`, `alert_sink.py`, `eventbridge_alert_sink.py`.
