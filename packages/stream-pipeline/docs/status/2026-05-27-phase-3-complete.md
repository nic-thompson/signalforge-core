...
**`DeviceRegistry`** (`stream_pipeline/detection/device_registry.py`):
...
**`MeanAggregation`** (`stream_pipeline/streaming/window_aggregator.py`):
...
**Feature emissions to `ProcessingResult.features`** (`stream_pipeline/streaming/realtime_pipeline.py`, `stream_pipeline/features/__init__.py`):
...
`FEATURE_SCHEMA_VERSION = "v1"` lives in `stream_pipeline/features/__init__.py` as a module-level constant. Versioning describes the feature set (which named features the pipeline produces), not the payload schema (which is versioned at the upstream contract layer).
