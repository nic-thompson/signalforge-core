...
247 tests passing. ruff and `mypy --strict` clean across 30 source files. CI is expected green at merge (ruff check, `mypy --strict` on `stream_pipeline`, pytest matrix on Python 3.11 and 3.12, test-count regression guard now at 247).

## What landed in Phase 4

**Dataset layer** (`stream_pipeline/datasets/`):
...
**Replay-deterministic identity** (`stream_pipeline/identity.py`):
...
- **`event-schema-contracts` ruff backlog** is unchanged by Phase 4's PR #4 (the new code is clean; the backlog is in existing files). `stream-pipeline`'s CI does not run ruff on the upstream. Note also: the upstream repo's CI runs `mypy` and `pytest` but no ruff job, so that backlog is not gating there.
