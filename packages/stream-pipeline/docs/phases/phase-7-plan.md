...
- `stream_pipeline/config/platform_settings.py` — the `for_replay()` method the driver threads through the builder.
- `stream_pipeline/streaming/realtime_pipeline.py` — the function-shaped `process_batch` the driver feeds, and the `RealtimePipeline` the builder returns.
