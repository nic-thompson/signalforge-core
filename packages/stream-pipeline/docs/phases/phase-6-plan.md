...
- `stream_pipeline/detection/device_registry.py`, `stream_pipeline/alerts/acknowledgement_registry.py` — the existing folds-over-a-stream the projections generalise.
- `stream_pipeline/streaming/realtime_pipeline.py` — `ProcessingResult`, what the projections consume.
- `stream_pipeline/config/platform_settings.py` — the `for_replay()` pattern Phase 6 extends.
