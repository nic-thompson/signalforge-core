...
**Package:** `signal_forge.datasets` (`PartitionKey` and the three partition
...
- `signal_forge/datasets/partition.py` — `PartitionKey` and the three
  partition helpers.
- `signal_forge/datasets/writer.py` — `DatasetWriter` protocol,
  `FlushedPartition`, the `_PartitionBufferSet` flush state machine, and
  `InMemoryDatasetWriter`.
- `signal_forge/datasets/serialisation.py` — `serialise_partition` and the
  byte-determinism knobs.
- `signal_forge/datasets/s3_writer.py` — `S3DatasetWriter`, object-key
