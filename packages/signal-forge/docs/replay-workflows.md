...
> **Status:** As-built (Phase 7). The replay driver is implemented in `signal_forge/replay/`. This document describes the realised shape; the determinism contracts the streaming layer guarantees (below) are unchanged from the Phase 1 stub, because they held exactly.
...
The replay driver is `run_replay`, in `signal_forge/replay/driver.py`. It is function-shaped over an injected event source and an injected pipeline-builder (working note D-20) — the same form the pipeline itself took (D-4), one level up:
...
`python -m signal_forge.replay <config.json>` is a thin parse-and-delegate shell over `run_replay`. It reads a JSON config, constructs settings and an event source from it, and delegates. The config shape is the one a Step Functions invocation passes:
...
- `signal_forge/replay/driver.py` — the `run_replay` driver.
- `signal_forge/replay/__main__.py` — the CLI shell and the config shape.
